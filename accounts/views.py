...
API_BASE_URL = getattr(settings, "CUSTOM_API_BASE_URL", "http://127.0.0.1:8000/")
...
def check_username_api(request):
    ...
    if not username:
        return Response(
            {"success": False, "message": "Debe proporcionar un nombre de usuario"},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def register_view(request):
    """
    Vista para el registro de usuarios usando API externa
    """
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            user_data = {
                ...
                # aquí enviamos password1 y password2 como espera tu API
            ...
            try:
                ...
                if response.status_code == 201:
                    ...
                    try:
                        ...
                        if "username" in error_data:
                            ...
                            form.add_error("password1", error_data["password"][0])
                        else:
                            form.add_error(
                                None, "Error en el registro. Verifica tus datos."
                            )
                    except Exception:
                ...
                # Si hay problema de conexión con la API
                ...
                # Si cualquier otra excepción se presenta
...
def login_view(request):
    ...
    if request.user.is_authenticated:
        messages.info(request, "Ya tienes una sesión activa.")
        return redirect("plista_productos")
                ...
                if response.status_code == 200:
                    ...
                    if user and user.is_active:
                        ...
                        if "access_token" in response_data:
                            ...
                            request.session["refresh_token"] = response_data.get(
                                "refresh_token", ""
                            )
                        ...
                        try:
                            ...
                            # Guardar tokens
                            if "access_token" in response_data:
                                request.session["api_token"] = response_data[
                                    "access_token"
                                ]
                                request.session["refresh_token"] = response_data.get(
                                    "refresh_token", ""
                            ...
                            next_url = request.GET.get(
                                "next", "productos:lista_productos"
                            )
                            return redirect(next_url)
                            ...
                            form.add_error(
                                None,
                                "Error al sincronizar usuario. Contacta al administrador.",
                            )
...
def logout_view(request):
    ...
    if username:
        messages.success(
            request, f"Has cerrado sesión exitosamente, {username}. ¡Hasta pronto!"
        )
    else:
